#filenames = ['C:/Users/zhoupenghua/Desktop/convedna.txt', 'C:/Users/zhoupenghua/Desktop/ldpcdna.txt', 'C:/Users/zhoupenghua/Desktop/polardna.txt','C:/Users/zhoupenghua/Desktop/bchdna.txt'] # 要读取的文件
import numpy as np
import pandas as pd
import torch
...
max_dataset_size_per_class = 10000 # 每类数据的最大样本数
...
class SequenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (
            torch.tensor(self.sequences[idx], dtype=torch.float),  # 输入数据为 float
            torch.tensor(self.labels[idx], dtype=torch.float)  # One-hot 标签
        )
...
def train_model(model, train_loader, test_loader, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # 为每个序列生成掩码
            mask = (batch_X != padding_value).float()  # 生成掩码，非填充位置为 1，填充为 0
            mask = mask.view(-1, 1)  # 扩展为二维

            loss = masked_loss(outputs, torch.argmax(batch_y, dim=1), mask)  # 使用 masked loss
            loss.backward()
            optimizer.step()
...
model = AdvancedCNNModel(input_channels=input_channels, sequence_length=max_length, num_classes=num_classes)
train_accuracy_list, test_accuracy_list, all_preds, all_targets = train_model(model, train_loader, test_loader,
                                                                              epochs=50)
end0 = time.time()
print(end0 - start0)
# 保存模型
torch.save(model.state_dict(), '0.4advanced_cnn_model100000.pth')
...
accuracy_data.to_csv('0.4advanced_cnn_model_accuracy_data100000.csv', index=False)
...
class_metrics.to_csv('0.4advanced_cnn_model_class_metrics100000.csv', index=False)

# 生成混淆矩阵
conf_matrix = confusion_matrix(all_targets, all_preds)
name=["Conve","LDPC","POLAR","BCH"]
# 可视化混淆矩阵
...
plt.savefig('0.4advanced_cnn_model_confusion_matrix100000.png')  # 保存混淆矩阵图像
plt.show()